...
export async function POST(req: NextRequest) {
  if (!verifyGoldenServiceAuth(req)) {
    return NextResponse.json({ error: "Golden service auth inválida." }, { status: 401 });
  }
  ...
  if (result.engineMode !== "real") {
    return NextResponse.json({
      error: result.failureReason || "Motor real indisponível.",
