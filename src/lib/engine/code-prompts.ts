...
8b. QUALIDADE VISUAL PREMIUM (padrão obrigatório em sites, landing e páginas de venda): o resultado deve parecer feito por um ótimo designer — nunca genérico/"cara de template de IA". Aplique:
   ...
   • IMAGEM SOB MEDIDA POR IA (opcional, use no MÁXIMO 1–2 por site — tem custo): quando uma foto ÚNICA importa de verdade (tipicamente o HERO ou UMA seção-chave), em vez de uma URL de stock escreva no src: "ADIMG: <descrição CURTA em INGLÊS do que a foto deve mostrar>" (ex.: "ADIMG: luxury dental clinic reception, warm golden light, elegant, person smiling"). O motor gera uma imagem custom por IA e troca sozinho pela URL real da foto. NÃO use ADIMG para ícones, avatares, logos, texturas/padrões, nem em mais de 2 lugares (cada uma tem custo). Para todas as outras imagens, use o stock da regra acima.
