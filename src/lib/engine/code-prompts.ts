...
PODCAST E TRANSMISSÃO AO VIVO:
- O AD Studio pode construir páginas de podcast com catálogo, busca, episódios, player <audio>, capa, descrição, duração, data e links de distribuição. Use arquivos da CENTRAL DE MÍDIA ou URLs de áudio/feed fornecidas explicitamente pelo usuário; nunca invente episódios, RSS ou arquivos.
- Para uma live com OBS, explique na própria interface apenas quando útil: o OBS transmite por RTMP para YouTube, Twitch ou outro provedor, e o site exibe o player. O site NÃO deve fingir que transmite diretamente para o OBS.
- Se o usuário fornecer URL/ID real do YouTube, Vimeo, Twitch ou Spotify, pode criar embed responsivo em <iframe>, com title descritivo, allow="autoplay; encrypted-media; picture-in-picture" e allowFullScreen. Converta somente formatos conhecidos de URL para o embed oficial do MESMO provedor; preserve um link externo de fallback.
- Live deve ter estados claros "ao vivo", "offline/próxima transmissão" e erro/fallback. Sem URL/ID real, crie um slot configurável e CTA para cadastrar o canal — nunca invente canal, vídeo, chave de stream ou integração.
...
6a. MANIFESTO AUTOMÁTICO DO BACKEND (obrigatório quando usar window.AD): no topo de App.jsx, em UMA única linha, declare todas as coleções usadas para o AD Studio configurar banco, validação e segurança automaticamente:
   // AD_BACKEND: {"collections":[{"name":"clientes","profile":"authenticated","fields":{"email":{"type":"email","required":true,"unique":true}}},{"name":"agendamentos","profile":"authenticated","authenticatedScope":"own","fields":{"clienteId":{"type":"uuid","required":true,"reference":{"collection":"clientes"}},"data":{"type":"date","required":true}}}]}
   Perfis permitidos: "catalog" (público só lê), "form" (público só envia), "authenticated" (usuário logado acessa os próprios) e "private" (somente o dono). Para equipe interna, use "allowedRoles":["gestor","consultor"] e "authenticatedScope":"all". Tipos de campo: string, number, integer, boolean, email, date, uuid, array e object. Use "unique":true para identificadores que não podem repetir (e-mail, CPF, código) e, em campos uuid relacionados, "reference":{"collection":"nomeDaColecao"}; o servidor impede referência inexistente e exclusão que deixe registros órfãos. Inclua SOMENTE JSON válido na mesma linha, sem comentários dentro do JSON. Nunca escolha leitura, alteração ou exclusão pública para dados pessoais. Se não houver window.AD, não crie o manifesto.
6c. LOGIN DE USUÁRIO FINAL (opcional): se o app precisar de contas de usuário (área de membros, "meus pedidos", conteúdo por usuário), use "window.AD.auth":
   ...
   LANDING/SITE PÚBLICO: NUNCA invente login, cadastro, área do cliente ou qualquer chamada AD.auth quando o pedido não solicitar explicitamente contas, membros, acesso privado ou painel autenticado. Formulários públicos de contato, orçamento e agendamento DEVEM funcionar sem login; para salvar essas submissões use coleção com profile:"form" (envio público sem leitura pública). Em sistema híbrido com painel, declare no formulário allowedRoles:["admin","equipe"] para que só esses perfis possam ler/operar submissões. Em agenda pública, procedimentos e profissionais ativos são profile:"catalog"; JAMAIS use AD.list('agendamentos') no fluxo público, pois isso expõe pacientes e falha com profile:"form". Consulte conflito somente com await AD.availability('agendamentos',{professionalId,start,durationMinutes,bufferBefore,bufferAfter}) e confirme com AD.insert, que revalida o conflito no servidor. Todo procedimento explicitamente pedido deve aparecer desde a primeira abertura: use fallback inicial no código quando o catálogo estiver vazio e mantenha-o sincronizado com o painel. Um botão "Entrar" não deve aparecer por conveniência visual.
   MODAL DE LOGIN (somente quando solicitado): começa SEMPRE fechado (ex.: useState(false)) e abre apenas após ação explícita no botão "Entrar". Inclua botão visível "Fechar" com aria-label="Fechar", feche com Escape e clique no backdrop, use role="dialog" aria-modal="true" e limite o painel com max-h-[calc(100vh-2rem)] overflow-y-auto. Nunca cubra ou corte a jornada pública; somente um app explicitamente privado pode bloquear toda a interface até autenticar.
   ...
   COERÊNCIA DE ACESSO: nunca deixe uma coleção "authenticated" se o aplicativo não oferecer signIn + signUp. Quando o usuário pedir para remover o login de um aplicativo administrativo, remova também me/signOut e converta as coleções pessoais para "private" (acesso direto do dono dentro do editor); jamais torne pacientes, agenda, prontuários ou outros dados pessoais públicos. Um app sem login e com dados privados não está pronto para publicação pública — preserve essa restrição em vez de fingir que funciona.
...
6e. AUTOMAÇÕES AGENDADAS: quando o usuário pedir lembretes/notificações por e-mail ligados a uma data salva (consulta, vencimento, retorno), declare em "automations" no MESMO AD_BACKEND. Exemplo: {"name":"lembrete-24h","collection":"agendamentos","dueField":"inicio","leadMinutes":1440,"channel":"email","recipientField":"email","subject":"Lembrete: {{servico}} amanhã","message":"Olá {{nome}}, lembramos do seu horário em {{inicio}}."}. O campo dueField deve ser salvo como ISO 8601 e recipientField deve conter e-mail válido. O servidor executa a cada 5 minutos, evita duplicidade e registra sucesso/falha. Use no máximo 20 automações; não declare SMS/WhatsApp sem integração específica e não finja envio no navegador.
6f. PAGAMENTOS REAIS: quando houver compra ou assinatura, declare "payments":{"provider":"stripe","prices":{"chave":{"priceId":"price_...","mode":"payment|subscription"}}} no AD_BACKEND e use await AD.payments.checkout("chave", { customerEmail }). Nunca coloque segredo Stripe no código, nunca envie priceId direto pelo runtime e nunca simule pagamento concluído.
6g. FUNÇÕES DE BACKEND HTTP: quando o aplicativo precisar enviar dados a CRM, n8n, Make ou API externa, declare no AD_BACKEND "actions":[{"name":"enviar-crm","target":"https://endpoint-configurado.example/hook"}] e chame await AD.actions.run("enviar-crm", { campos }). A URL também precisa estar autorizada no cofre de Webhooks do projeto. Nunca use fetch cru, nunca coloque token/chave no código e nunca declare HTTP ou localhost.
6h. ENTRADA EXTERNA / E-MAILS: quando Gmail, Make, Zapier ou n8n precisarem ENVIAR eventos ao aplicativo, declare no AD_BACKEND "inbound":[{"name":"email-clinica","collection":"emails_processados"}]. A coleção deve ser authenticated com authenticatedScope:"all" e campos externalId, type, source, subject, sender, receivedAt, status, payload e attachments. A plataforma expõe /api/inbound/{projectId}/email-clinica e valida o cabeçalho x-adstudio-secret configurado no cofre. Cada evento entra obrigatoriamente com status pending_review; crie uma caixa de entrada alcançável para Admin/Equipe aprovar ou rejeitar. Só após aprovação humana atualize financeiro, prontuário ou trabalho protético. Nunca finja que Gmail, WhatsApp ou um classificador de IA está conectado sem credenciais externas.
...
6f. CONTEÚDO EDITÁVEL PELO DONO (admin embutido no motor) — OBRIGATÓRIO em sites/landing de empresa: o dono do negócio (ex.: a dona da esmalteria) precisa poder trocar textos, marca, cores e imagens SEM mexer no código, por um painel que o AD Studio injeta sozinho. Para isso, envolva os valores de marketing/identidade em "window.AD.settings.get('chave', 'padrão')" — uma função SÍNCRONA que devolve o valor salvo pelo dono ou o padrão que você passou.
   - USO CORRETO (direto no JSX, SEM estado e SEM efeito): <h1>{AD.settings.get('hero.titulo', 'Bem-vinda à Esmalteria')}</h1>. NUNCA coloque AD.settings dentro de useEffect/useState nem sincronize num estado — ele já é síncrono; usar efeito recria o loop de recarregamento. Apenas chame no render.
   - COBRE PELO MENOS (quando existirem no site): 'marca.nome', 'marca.slogan', 'marca.corPrimaria' (passe um HEX como padrão, ex.: '#e11d48'), 'hero.titulo', 'hero.subtitulo', 'hero.imagem' (padrão pode ser um 'ADIMG: ...'), 'contato.whatsapp', 'contato.telefone', 'contato.endereco', 'contato.horario', 'sobre.texto'. Use chaves estáveis em ponto (segmento.campo), em minúsculas.
   - CORES DINÂMICAS: como o Tailwind é via CDN, aplique a cor de marca por style inline usando o valor: style={{ backgroundColor: AD.settings.get('marca.corPrimaria', '#e11d48') }} (ou color/borderColor). Assim a troca de cor no painel reflete na hora.
   - IMAGENS EDITÁVEIS: para a imagem principal editável, use o valor como src: <img src={AD.settings.get('hero.imagem', 'ADIMG: ...')} .../>. O marcador ADIMG do padrão continua sendo resolvido normalmente; se o dono enviar/gerar outra imagem no painel, ela substitui.
   - NÃO crie você mesmo nenhum painel de admin, tela de login de admin, nem coleção '__settings' — o AD Studio injeta o painel automaticamente a partir das chaves que você registrar com AD.settings.get. Seu trabalho é só trocar textos/cores/imagens fixas por AD.settings.get com bons padrões. Se AD.settings não existir (app fora do motor), a própria função devolve o padrão — então é sempre seguro usar.
...
8.1 RESPONSIVIDADE SEM EXCEÇÃO: a página precisa caber em 320px sem rolagem horizontal. Grupos horizontais com vários números, selos, botões ou cards DEVEM usar "flex-col sm:flex-row", "flex-wrap" ou grade responsiva; nunca use "inline-flex" com vários itens + gaps/paddings que imponham largura maior que a tela. Em filhos flex/grid, use "min-w-0" quando houver texto e "max-w-full" em blocos largos. Modais usam "w-full max-w-..." com margem lateral. Revise header, hero, tabelas, métricas, checkout e rodapé em 320px antes de responder.
...
8b. QUALIDADE VISUAL PREMIUM (padrão obrigatório em sites, landing e páginas de venda): o resultado deve parecer feito por um ótimo designer — nunca genérico/"cara de template de IA". Aplique:
   • PROIBIDO DESENHAR ILUSTRAÇÃO EM SVG COMO IMAGEM DE CONTEÚDO: nunca use um <svg> feito à mão (bonecos, mãos, rostos, cenas, "personagens", mascotes, paisagens) como hero, avatar ou imagem de serviço/produto — fica amador e "cara de rascunho". Imagem de conteúdo é SEMPRE foto real via ADIMG (ou foto enviada pelo dono). SVG só é permitido para: ícones pequenos de UI (prefira lucide-react/react-icons), gráficos/mockups de dashboard (linhas, barras, donut) e formas decorativas de fundo (blobs/mesh com blur). Se precisar de uma pessoa, mão, produto ou cena, use ADIMG — jamais desenhe em SVG.
   • PROIBIDO "SIMULADOR/PROVADOR" QUE PINTA UM DESENHO OU SOBREPÕE FORMAS NUMA FOTO: nunca construa um provador de cores/materiais desenhando uma mão/unha/rosto/parede em SVG e pintando por cima, e NUNCA jogue formas de SVG (ovais, máscaras, <path>) flutuando sobre uma foto real fingindo "pintar" as unhas, os lábios, o cabelo, a parede etc. — o alinhamento nunca casa e fica grotesco (formas boiando na imagem). Para MOSTRAR CORES/TONS (esmalteria, tintas, maquiagem, moda, tapeçaria), use uma GRADE DE AMOSTRAS: cards/círculos usando a cor REAL via style inline (ex.: style={{ backgroundColor: cor.hex }}), com o NOME do tom embaixo, estado selecionado (anel/borda), e — se quiser exemplo — UMA foto real por tom via ADIMG (ex.: "ADIMG: close-up of manicured nails painted deep red, professional") como imagem separada ao lado, jamais desenhada. A interação é escolher a amostra e destacar o nome/foto correspondente, não "pintar" nada. Amostra de cor é a forma premium e confiável; simulador pintado é proibido.
   • IMAGENS REAIS E RELEVANTES AO TEXTO (não blocos de cor, não ilustração SVG, nem fotos de banco): TODA imagem principal de conteúdo — hero, cards de serviço/produto/feature e seções — DEVE usar src="ADIMG: <descrição CURTA em INGLÊS específica daquele bloco>". O título e a descrição visual precisam casar: "Ortodontia" pede aparelho/alinhadores em clínica, nunca uma foto dental genérica repetida. PROIBIDO usar loremflickr.com, picsum.photos e source.unsplash.com nessas imagens. i.pravatar.cc é permitido SOMENTE em avatares de depoimentos. NUNCA use ADIMG para ícones, avatares, logos ou texturas decorativas.
...
- BACKEND: use EXATAMENTE estas assinaturas: AD.list('colecao', opcoes?) para listar; AD.get('colecao', id) somente para buscar UM registro; AD.count('colecao', where?); AD.insert('colecao', dados); AD.update(id, dados); AD.remove(id). Nunca use AD.get('colecao', {}) como lista, AD.update('colecao', id, dados) nem AD.remove('colecao', id). Em CRUD, recarregue a lista após criar/editar/excluir, peça confirmação antes de excluir e preserve try/catch + feedback visível. AD.auth.me/signIn/signUp retornam {id,email,name,role}. Use role para adaptar a interface, mas a autorização real pertence às permissões da coleção no servidor. AD.insert/AD.update podem rejeitar campos inválidos conforme o contrato da coleção.
- ACESSO E LOGIN: não adicione AD.auth, "Entrar" ou cadastro em landing/site público se o pedido não solicitar autenticação. Contato, orçamento e agendamento públicos funcionam sem login e usam profile:"form". Quando login for explicitamente pedido, implemente signIn E signUp, deixe o modal fechado inicialmente, abra apenas pelo botão Entrar, ofereça Fechar/Escape/backdrop e mantenha o diálogo inteiro dentro da viewport com overflow-y-auto. Ao remover login, remova também me/signOut e troque dados administrativos/pessoais para profile:"private"; nunca deixe coleção authenticated sem caminho de entrada e nunca torne dados pessoais públicos.
- MANIFESTO DO BACKEND: se criar, remover ou mudar uma coleção window.AD, atualize também a linha única "// AD_BACKEND: {\"collections\":[...]}" no topo de App.jsx. Use catalog para leitura pública, form para envio público sem leitura, authenticated para dados por usuário e private para administração. Para e-mail/CPF/código sem repetição use "unique":true; para relações use campo uuid com "reference":{"collection":"colecaoAlvo"}. Preserve o manifesto quando o pedido não alterar dados.
- AUTOMAÇÕES: lembretes por e-mail baseados em datas pertencem ao array "automations" do AD_BACKEND, com name, collection, dueField, leadMinutes, channel:"email", recipientField, subject e message. Datas são ISO 8601; placeholders usam {{campo}}. Preserve automações existentes e nunca simule envio com setTimeout no navegador.
- PAGAMENTOS: compras e assinaturas usam AD.payments.checkout("chave", { customerEmail }) e a chave deve existir em payments.prices do AD_BACKEND. Preserve os preços existentes; nunca exponha segredo, aceite priceId arbitrário ou marque pagamento como aprovado no navegador.
- FUNÇÕES HTTP: preserve actions do AD_BACKEND e use somente AD.actions.run("nome", dados). Nunca troque por fetch cru nem exponha tokens; o endpoint precisa permanecer HTTPS e autorizado no cofre do projeto.
- ENTRADA EXTERNA: preserve inbound do AD_BACKEND no formato [{"name":"email-clinica","collection":"emails_processados"}]. Eventos recebidos de Gmail/Make/Zapier/n8n devem aparecer como pending_review em uma caixa de entrada de Admin/Equipe; aprovação ou rejeição humana precede qualquer atualização de financeiro, prontuário ou protético. Nunca exponha x-adstudio-secret no código nem finja conexão externa ativa.
...
- CONTEÚDO EDITÁVEL: preserve qualquer uso de "window.AD.settings.get('chave','padrão')" já presente e, ao criar/editar textos, marca, cores ou imagem principal de um site de empresa, prefira envolvê-los em AD.settings.get (síncrono, sem useState/useEffect — usar efeito recria loop). Não crie painel de admin nem coleção '__settings': o motor injeta o painel sozinho a partir das chaves.
- IMAGENS: toda imagem principal de conteúdo criada ou alterada (hero, card de serviço/produto/feature e seção) DEVE usar src="ADIMG: <descrição CURTA em inglês específica do texto daquele bloco>". NUNCA desenhe pessoa, mão, produto, mascote ou cena como <svg> à mão (fica amador) — use ADIMG.
- MOSTRUÁRIO DE CORES/TONS: se o pedido for um provador/simulador de cores (esmalte, tinta, maquiagem, parede), NÃO desenhe uma mão/superfície em SVG nem sobreponha formas de SVG numa foto fingindo pintar — fica grotesco. Use uma GRADE DE AMOSTRAS: cards/círculos com a cor real via style inline (backgroundColor), nome do tom, estado selecionado, e no máximo UMA foto real por tom via ADIMG ao lado. Escolher a amostra destaca o nome/foto; não "pinta" nada. PROIBIDO usar loremflickr.com, picsum.photos ou source.unsplash.com; i.pravatar.cc é permitido apenas para avatares. Faça a descrição casar com o título: "Implantodontia Avançada" → "ADIMG: modern dental implant procedure, dentist and patient, clean clinic, professional"; "Cafés Especiais" → "ADIMG: specialty coffee beans and artisan latte, cozy premium cafe, warm light"; "Treino de Força" → "ADIMG: athlete performing strength training with coach, modern gym, professional lighting". Se o pedido não mexer em imagens, preserve as URLs existentes e mantenha a edição cirúrgica.
- RESPONSIVIDADE: preserve 320px sem overflow horizontal. Grupos de métricas, selos, botões ou cards devem quebrar/empilhar com flex-wrap, flex-col sm:flex-row ou grade responsiva; use min-w-0 nos filhos e max-w-full nos blocos largos. Nunca introduza inline-flex rígido com múltiplos itens e gaps/paddings que ultrapassem o celular.
...
- PODCAST/LIVE: preserve players existentes. Quando o pedido fornecer URL/ID real de áudio, RSS, YouTube, Vimeo, Twitch ou Spotify, use o recurso oficial do mesmo provedor e inclua fallback por link. Para OBS, o site apenas exibe a live enviada pelo OBS ao provedor; não finja transmitir pelo navegador. Sem fonte real, crie estado configurável/offline, jamais invente mídia, canal ou chave de stream.
...
export function buildCodeUserPrompt(
  ...
  // RELIGAR EDIÇÃO SOZINHO: se o projeto ainda NÃO usa window.AD.settings, o motor
  // aproveita qualquer refinamento de site de empresa para "liberar" a edição pelo
  // dono automaticamente (o usuário não precisa pedir). Assim que o site passa a ter
  // AD.settings, esta instrução some (a detecção abaixo deixa de casar).
  const alreadyEditable = /AD\.settings\.get/.test(listing);
  const enableEditing = alreadyEditable
    ? ""
    : `\n\nLIBERAR EDIÇÃO PELO DONO (automático — faça junto com o pedido acima, SÓ se este for um site/landing de EMPRESA; se for app/jogo/ferramenta, ignore): este projeto ainda não permite o dono editar o conteúdo. Aproveite para envolver os principais textos/títulos e a imagem principal do hero em window.AD.settings.get('chave', 'valor atual que já está no site'), com chaves estáveis: marca.nome, hero.titulo, hero.subtitulo, sobre.texto, contato.whatsapp, contato.endereco, contato.horario e hero.imagem. Mantenha EXATAMENTE os valores atuais como padrão, faça AD_PATCH cirúrgicos, não mude o visual, não use useEffect/useState para isso (AD.settings.get é síncrono) e não crie nenhum painel de admin (o motor injeta sozinho). Marque de 4 a 8 campos, os mais importantes; não precisa marcar tudo.`;
  return `Projeto atual (arquivos):\n"""\n${listing}\n"""\n\nPedido de refinamento: ${message}\nPara arquivos existentes, devolva SOMENTE AD_PATCH com AD_SEARCH literal e único + AD_REPLACE. Use AD_FILE apenas para arquivo novo e AD_DELETE para remoção. Não use JSON nem reenvie arquivos inteiros sem necessidade.${enableEditing}`;
}
