  ...
  return (
    <div className="flex h-12 items-center justify-between border-b bg-background px-3">
      ...
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon" onClick={undo} disabled={past.length === 0} aria-label="Desfazer" title="Desfazer (⌘Z)"><Undo2 /></Button>
        <Button variant="ghost" size="icon" onClick={redo} disabled={future.length === 0} aria-label="Refazer" title="Refazer (⌘⇧Z)"><Redo2 /></Button>
        {studio && (
          <Button variant="ghost" size="icon" onClick={() => setDetailsOpen(true)} aria-label="Detalhes de produção" title="Briefing / observações internas">
            <ClipboardList />
          </Button>
        )}
        ...
        <Button variant="ghost" size="icon" onClick={onToggleEditor} aria-label="Alternar editor" title="Editor visual" className="hidden lg:inline-flex"><PanelRight /></Button>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input defaultValue={meta.client ?? ""} onBlur={(e) => onMetaChange({ client: e.target.value })} placeholder="Ex.: Clínica Sorriso Perfeito" />
            </div>
            <div className="space-y-2">
              <Label className="text-xs">Observações internas</Label>
              <Textarea defaultValue={meta.notes ?? ""} onBlur={(e) => onMetaChange({ notes: e.target.value })} rows={5} placeholder="Escopo, prazos, pendências, o que o cliente pediu…" />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              ...
              <Switch checked={!!meta.whitelabel} onCheckedChange={(v) => onMetaChange({ whitelabel: v })} />
            </div>
          </div>
          <DialogFooter><Button variant="brand" onClick={() => setDetailsOpen(false)}>Fechar</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
