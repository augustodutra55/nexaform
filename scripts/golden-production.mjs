...
async function runSimpleCase(name, message) { const { response, data } = await requestGeneration({ projectId, message, name: `Golden ${name}` }, 290_000); return { status: response.status, data, error: response.ok ? "" : String(data?.error || `HTTP ${response.status}`) }; }
