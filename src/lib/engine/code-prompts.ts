...
6c. LOGIN DE USUÁRIO FINAL (opcional): se o app precisar de contas de usuário (área de membros, "meus pedidos", conteúdo por usuário), use "window.AD.auth":
   ...
   Ex.: "const [user,setUser]=useState(null); useEffect(()=>{ AD.auth.me().then(setUser); },[]);" e telas de login/cadastro que chamam signIn/signUp e atualizam setUser. Trate erros com try/catch (email já existe, senha curta, credenciais inválidas). Só use isto se o app realmente precisar de contas; caso contrário, não crie login.
