...
function customPermissions(raw: any): CollectionPermissions {
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  return NextResponse.json({ settings: data ?? PRIVATE_PERMISSIONS, configured: !!data });
}
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const settings = profile === "custom" ? customPermissions(body?.permissions) : PROFILES[profile];
  const { data, error } = await context.admin!
    .from("app_collection_settings")
    .upsert(
      { project_id: projectId, collection, ...settings },
